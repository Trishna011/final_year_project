...
#df_real = pd.read_csv("processed_data/real_train_val_preprocessed2.csv")
#df_real = pd.read_csv("processed_data/real_test_preprocessed2.csv")
...
#df_extracted.to_csv("processed_data/real_train_val_with_extracted_features_synonyms2.csv", index=False)
# df_extracted.to_csv("processed_data/real_test_with_extracted_features_synonyms2.csv", index=False)

# df_final = extract_reno_cost()

# -------------------------
# extra encoding needed for non catboost/lightGBM models
# -------------------------

#df_real_extracted = pd.read_csv("processed_data/real_train_val_with_predicted_reno_cost.csv")
df_real_extracted = pd.read_csv("processed_data/real_test_with_predicted_reno_cost.csv")
...
df_real_extracted["material_grade"] = df_real_extracted["material_grade"].map(material_mapping)

#encode renovation_type
def normalize_reno(x):
...
df_real_extracted["type_of_renovation"] = df_real_extracted["type_of_renovation"].apply(normalize_reno)

output_rows = []

for idx, row in df_real_extracted.iterrows():
    base = {
    "source_row": idx,
    "property_size": row["property_size"],
    "location": row["location"],
    "renovation_cost": row.get("renovation_cost", np.nan),
    }

    rooms = row["type_of_renovation"]
    ...
    for room in rooms:
        ...
        output_rows.append({
            **base,
            "unit_type": room_lower,
            "sqft_renovated": row["sqft_renovated"],
            "sqft_to_add": row["sqft_to_add"],
            "material_grade": row["material_grade"],
            "structural_change": row["structural_changes"],

            "reno_bathroom": int(room_lower == "bathroom"),
            "reno_bedroom": int(room_lower == "bedroom"),
            "reno_kitchen": int(room_lower == "kitchen"),
            "reno_living_room": int(room_lower == "living room"),
            "reno_other_custom": int(room_lower == "other/custom"),
            "reno_full_renovation": int(room_lower == "full renovation"),
        })
    df_real_extracted = pd.DataFrame(output_rows)

df_real_extracted.to_csv("processed_data/real_test_encoded_with_predicted_reno_cost.csv", index=False)